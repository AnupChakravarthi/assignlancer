...
class CustomersAuthentication {
  ...
  function query_validate_emailRegOrNot($email){
   $sql="SELECT count(*) FROM accounts WHERE email='".$email."' AND accountType='CUSTOMERS';";
   return $sql;
  }
  ...
  function query_check_emailDuplicateExists($email,$account_Id){
   ...
   $sql.="accountType='CUSTOMERS';";
   return $sql;
  }
  
  function query_validate_UserAuthentication($email,$acc_pwd){
   $sql="SELECT * FROM accounts WHERE email='".$email."' AND acc_pwd='".md5($acc_pwd)."' AND accountType='CUSTOMERS';";
   return $sql;
  }
  ...
  function query_addAccount($account_Id,$availStatus,$name,$email,$acc_pwd,$createdOn,$country,$time_Id){
	...
	$sql.="time_Id) VALUES ('".$account_Id."','CUSTOMERS','".$availStatus."','".$name."','".$email."','".$acc_pwd."','";
  ...
  function query_updateAccount($account_Id,$availStatus,$name,$email,$email_val,$country){
	...
	$sql=chop($sql,',');
	$sql.=" WHERE account_Id='".$account_Id."' AND accountType='CUSTOMERS';";
    return $sql; 
  }
  
  function query_updatePwdAccount($account_Id,$oldPassword,$newPassword){
    $sql="UPDATE accounts SET acc_pwd='".md5($newPassword)."' ";
	$sql.="WHERE account_Id='".$account_Id."' AND acc_pwd='".md5($oldPassword)."' AND accountType='CUSTOMERS';";
	return $sql; 
  }
  
  function query_deleteAccount($account_Id){
    $sql="DELETE FROM accounts WHERE account_Id='".$account_Id."' AND accountType='CUSTOMERS';";
	return $sql; 
  }
  
  function query_count_getListOfAccount(){
    $sql="SELECT count(*) FROM accounts WHERE accountType='CUSTOMERS';";
	return $sql; 
  }
  function query_data_getListOfAccount($limit_start,$limit_end){
    $sql="SELECT * FROM accounts WHERE accountType='CUSTOMERS' LIMIT ".$limit_start.",".$limit_end.";";
	return $sql; 
  }
}
